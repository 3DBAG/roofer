...
namespace RasterTools {
  ...
  class Raster {
   public:
    ...
    // rasterise a polygon and return a list with points - one in the center of
    // each pixel inside the polygon in the polygon first point is *not*
    ...
    std::vector<point3d> rasterise_polygon(T &polygon,
      ...
      for (pixelY = IMAGE_TOP; pixelY < IMAGE_BOT; pixelY++) {
        std::vector<int> intersect_x;  // vector to hold the x-coordinates where
                                       // the scanline intersects the polygon
    ...
    std::vector<point3d> rasterise_polygon(T &polygon,
                                           bool returnNoData = true) const {
      return rasterise_polygon(polygon, {0, 0}, {double(dimx_), double(dimy_)},
                               returnNoData);
    };
