  ...
  virtual void after_split_face(Face_handle old_face, Face_handle new_face,
    ...
    if (n_faces == 1)
      ...
      if (!is_hole &&
        ...
        std::cout << "Ignored input footprint hole that is touching footprint "
                     "exterior\n";
...
class Snap_observer : public CGAL::Arr_observer<Arrangement_2> {
 public:
  Snap_observer(Arrangement_2& arr) : CGAL::Arr_observer<Arrangement_2>(arr){};

  virtual void after_create_edge(Halfedge_handle e) { e->data().blocks = true; }
  virtual void after_split_face(Face_handle old_face, Face_handle new_face,
                                bool) {
...
bool arrangementface_to_polygon(Face_handle face, roofer::LinearRing& polygons,
                                double h = 0);
