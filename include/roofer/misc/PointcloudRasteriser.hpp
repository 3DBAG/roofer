...
namespace roofer::misc {
   ...
   * Each cell in the layer 'grp' gives probability for it to be a glass roof cell
   * The closer the value is to 1, the cell contains mostly points belonging to one
   * class, either ground or non-ground. The closer the value is to 0, the cell contains
   * a mix of points from both classes, which is a characteristic of a glass roof.
   * The function computes the average and the mean value of the 'grp' layer and 
   * if they are below the threshold values, the point cloud is classified as a glass
   * roof.
   * 
   * @param image_bundle The ImageMap containing multiple raster layers, including 'grp'.
   * @return bool Returns true if the analysis suggests the presence of a glass roof;
   *              otherwise, false.
   */
