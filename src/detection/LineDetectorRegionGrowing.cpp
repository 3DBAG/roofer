...
inline size_t DetectLinesNode::detect_lines_ring(linedect::LineDetector& LD, Plane& plane, SegmentCollection& segments_out) {
...
void DetectLinesNode::process(){
  ...
  for (size_t i=0; i<input_geom.size(); ++i) {
    ...
    auto n_detected = detect_lines_ring(LD, planes[plane_id].first, edge_segments);
