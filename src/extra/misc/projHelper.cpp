...
namespace roofer::misc {
  ...
  struct projHelper : public projHelperInterface {
    ...
    void proj_construct() override { projContext = proj_context_create(); };
    void proj_clone_from(
        const projHelperInterface& other_proj_helper) override {
      const projHelper* other_proj =
          static_cast<const projHelper*>(&other_proj_helper);
      data_offset = *other_proj->data_offset;
      projContext = proj_context_clone(other_proj->projContext);
      if (other_proj->processCRS)
        processCRS = proj_clone(projContext, other_proj->processCRS);
      if (other_proj->projFwdTransform)
        projFwdTransform =
            proj_clone(projContext, other_proj->projFwdTransform);
      if (other_proj->projRevTransform)
        projRevTransform =
            proj_clone(projContext, other_proj->projRevTransform);
    };
    ...
    arr3f coord_transform_fwd(const double& x, const double& y,
      ...
      auto result = arr3f{float(coord.xyz.x - (*data_offset)[0]),
    ...
    arr3d coord_transform_rev(const float& x, const float& y,
      ...
      if (!data_offset.has_value()) {
        ...
        coord = proj_coord(x + (*data_offset)[0], y + (*data_offset)[1],
                           z + (*data_offset)[2], 0);
      };
    ...
    void set_process_crs(const char* crs) override {
      ...
      if (!processCRS)
        throw rooferException("Unable to create CRS from string: " +
                              std::string(crs));
    };
    void set_fwd_crs_transform(
        const char* source_crs,
        bool normalize_for_visualization = false) override {
      if (processCRS) {
        sCRS = proj_create(projContext, source_crs);
        if (!sCRS)
          throw rooferException("Unable to create source CRS from string: " +
                                std::string(source_crs));

        if (normalize_for_visualization)
          sCRS = proj_normalize_for_visualization(projContext, sCRS);

        projFwdTransform =
            proj_create_crs_to_crs_from_pj(projContext, sCRS, processCRS, 0, 0);

        if (!projFwdTransform)
          throw rooferException("Unable to create forward transformation.");
      } else {
        // std::cout << "Unable to create CRS transform, process CRS is
        // undefined\n";
      }
    };
    void set_rev_crs_transform(
      ...
      if (processCRS) {
        ...
        if (normalize_for_visualization)
          tCRS = proj_normalize_for_visualization(projContext, tCRS);

        projRevTransform =
            proj_create_crs_to_crs_from_pj(projContext, processCRS, tCRS, 0, 0);
    ...
    std::string get_rev_crs_id_auth_name() override {
      ...
      if (tCRS) auth_name = proj_get_id_auth_name(tCRS, 0);
      return auth_name;
    };
    std::string get_rev_crs_id_code() override {
      std::string code;
      if (tCRS) code = proj_get_id_code(tCRS, 0);
      return code;
    };
