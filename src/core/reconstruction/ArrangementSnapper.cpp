...
namespace roofer::detection {
  ...
  void get_incident_constraints(T& tri, Vertex_handle& vthis,
                                Vertex_handle& vexcept1, Vertex_handle vexcept2,
    ...
    if (ec != nullptr) {
      do {
        if (tri.is_constrained(*ec)) {
          ...
          // it. vmirror is the vertex opposing ec in face_ccw face_ccw will be
          // collapsed if vmirror is one of the vexcept vertices auto i =
          // face_ccw->index(vthis); auto vmirror =  face_ccw->vertex( tri.cw(i)
          // ); if (
          //   vmirror == vexcept1 ||
          //   vmirror == vexcept2
          // ) {
  ...
  void restore_constraints(T& tri, T::Point_2& pnew,
    ...
    for (auto& [vh, finfo] : constraints_to_restore) {
      ...
      //         fc->info() = finfo;
      //       } else {
      //         // fc is cw to edge, so find the neighbor on the other side of
      //         edge fc->neighbor( tri.cw( fc->index(vh) ) )->info() = finfo;
      //       }
      //     }
      //   } while (++fc != done);
      // }
  ...
  class ArrangementSnapper : public ArrangementSnapperInterface {
    void compute(Arrangement_2& arr, ArrangementSnapperConfig cfg) override {
      ...
      for (auto fit : tri.finite_face_handles()) {
        ...
        if (auto f =
                boost::get<Face_const_handle>(&obj)) {  // located inside a face
          // std::cout << "inside "
        ...
        // else if (auto e = boost::get<Halfedge_const_handle>(&obj)) // located
        // on an edge
      ...
      // for (auto fh = tri.finite_faces_begin(); fh != tri.finite_faces_end();
      // ++fh) {
      ...
      //     (fh->vertex(2)->point().y()), 0}; triangles_og.push_back({ p0,p1,p2
      ...
      // Detect triangles with 1 short edge  => collapse the short edge to point
      // (remove one vertex)
