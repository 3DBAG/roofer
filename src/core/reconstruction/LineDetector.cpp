...
namespace roofer::detection {
  ...
  inline size_t detect_lines_ring(linedect::LineDetector& LD,
                                  const Plane& plane,
                                  SegmentCollection& segments_out,
                                  roofer::detection::LineDetectorConfig& cfg) {
    ...
    if (LD.segment_shapes.size() > 1) {
      ...
      if (cfg.perform_chaining) {
        ...
        auto chained_ring_pts = linereg::chain_ring<SCK>(
            idx, SCK::Plane_3(plane.a(), plane.b(), plane.c(), plane.d()),
