...
size_t LineDetector::get_bounded_edges(roofer::SegmentCollection& edges) {
  ...
  for (auto seg : segment_shapes) {
    ...
    ordered_segments[id_min][0] = {float(p0.x()), float(p0.y()), float(p0.z())};
    ordered_segments[id_min][1] = {float(p1.x()), float(p1.y()), float(p1.z())};
  }
...
inline Line LineDetector::fit_line(vector<size_t>& neighbour_idx) {
  ...
  Line line;
  linear_least_squares_fitting_3(neighbor_points.begin(), neighbor_points.end(),
                                 line, CGAL::Dimension_tag<0>());
  return line;
}
...
bool LineDetector::grow_region(size_t seed_idx) {
  ...
  while (candidates.size() > 0) {
    ...
    for (auto n_id : neighbours[candidate_id]) {
      ...
      // point_seed_flags[n_id] = false; // this point can no longer be used as
      // seed
