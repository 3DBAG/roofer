...
namespace roofer::detection {
  ...
  class Val3dator : public Val3datorInterface {
    void validate(std::vector<std::unique_ptr<val3dity::Surface>>& surfaces,
                  Val3datorConfig cfg) {
      ...
      for (auto& sh : surfaces) {
        ...
        // if(log_invalids) {
        //   if(errorpts.size()) {
        //     std::cout << "Val3dity error points (" << errorpts.size() << "
        //     total):\n"; std::cout << std::fixed << std::setprecision(3);
        //   }
        // }
        for (const auto& p : errorpts) {
          error_locations.push_back(
              arr3f{float(p.x()), float(p.y()), float(p.z())});
          // if(log_invalids) {
          //   if(errorpts.size()) std::cout << "\t" << (p.x()+offset[0]) << " "
    ...
    void compute(const std::unordered_map<int, Mesh>& meshmap,
      ...
      // init val3dity
      // val3dity::Primitive::set_translation_min_values((*manager.data_offset())[0],
      // (*manager.data_offset())[1]);
      // val3dity::Surface::set_translation_min_values((*manager.data_offset())[0],
      // (*manager.data_offset())[1]);
    ...
    void compute(const TriangleCollection& triangles,
                 Val3datorConfig cfg) override {
      // init val3dity
      // val3dity::Primitive::set_translation_min_values((*manager.data_offset())[0],
      // (*manager.data_offset())[1]);
      ...
      for (auto& sh : surfaces) {
        for (auto sfid : sh->get_error_face_ids()) {
          try {
            int fid = stoi(sfid) - 1;
            // std::cout << "number of faces: " << faces.size() << "; error: "
            // << fid << "\n";
