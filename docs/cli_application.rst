...
Example config file
-------------------
Below is an example of a `TOML <https://toml.io/en/>`_ configuration file for the :program:`roofer` CLI application. It shows all the available options. Noticed that many of these options are also available as command line arguments, incase one option is provided both in the configuration file and as a command line argument, the command line argument takes precedence.
