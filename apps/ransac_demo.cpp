...
void myCallback() {
  ...
  if (ImGui::Button("Do RANSAC")) {
    ...
    for (size_t i = 1; i < nShapes; i++) {
      randColor[i] = {{polyscope::randomUnit(), polyscope::randomUnit(),
...
int main(int argc, const char* argv[]) {
  ...
  // "/home/ravi/git/gfc-building-reconstruction/single/test-data/wippolder.gpkg";
  // std::string las_source =
  // "/home/ravi/git/gfc-building-reconstruction/single/test-data/wippolder.las";
  // std::string vector_file =
  // "/mnt/Data/LocalData/Kadaster/true_ortho_experimenten/2021_LAZ_Leiden_Almere/DenHaag/bag_83000_455000.gpkg";
  // std::string las_source =
  // "/mnt/Data/LocalData/Kadaster/true_ortho_experimenten/2021_LAZ_Leiden_Almere/DenHaag/83000_455000.laz";
  std::string vector_file =
      "/mnt/Data/LocalData/Kadaster/true_ortho_experimenten/2021_Eindhoven/LAZ/"
      "DSM_1500_4142.gpkg";
  std::string las_source =
      "/mnt/Data/LocalData/Kadaster/true_ortho_experimenten/2021_Eindhoven/LAZ/"
      "DSM_1500_4142.laz";
  ...
  PointCloudCropper->process(las_source, polygons, polygons, point_clouds,
                             ground_elevations);
  ...
  // polyscope::getSurfaceMesh("my mesh")->addVertexScalarQuantity("my_scalar",
  // scalarQuantity); polyscope::getSurfaceMesh("my
  // mesh")->addFaceVectorQuantity("my_vector", vectorQuantity);
