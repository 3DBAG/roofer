...
std::unordered_map<int, roofer::Mesh> extrude(
    Arrangement_2 arrangement, float floor_elevation,
    ...
    roofer::detection::PlaneDetectorInterface* PlaneDetector, LOD lod) {
  ...
  rec.log(worldname + "ArrangementDissolver",
          rerun::LineStrips3D(roofer::detection::arr2polygons(arrangement)));
  ...
  // roofer::detection::arr2polygons(arrangement) ));
  ...
  logger.info(fmt::format("Completed PC2MeshDistCalculator. RMSE={}",
...
int main(int argc, const char* argv[]) {
  ...
  std::string path_pointcloud =
      "output/wippolder/objects/503100000000296/crop/"
      "503100000000296_pointcloud.las";
  std::string path_footprint =
      "output/wippolder/objects/503100000000296/crop/503100000000296.gpkg";
  ...
  logger.info(fmt::format("{} ground points and {} roof points", points_ground.size(),
               points_roof.size()));
  ...
  logger.info(fmt::format("Completed PlaneDetector (roof), found {} roofplanes",
  ...
  logger.info(fmt::format("Completed PlaneDetector (ground), found {} groundplanes",
  ...
  if (skip) {
    auto SimplePolygonExtruder =
        roofer::detection::createSimplePolygonExtruder();
    ...
    logger.info(fmt::format("Completed AlphaShaper (roof), found {} rings, {} labels",
    ...
    logger.info(fmt::format("Completed AlphaShaper (ground), found {} rings, {} labels",
    ...
    auto multisolids_lod12 = extrude(
        arrangement, floor_elevation, attr_val3dity_lod12, attr_rmse_lod12,
        SegmentRasteriser.get(), PlaneDetector.get(), LOD12);
    ...
    auto multisolids_lod13 = extrude(
        arrangement, floor_elevation, attr_val3dity_lod13, attr_rmse_lod13,
        SegmentRasteriser.get(), PlaneDetector.get(), LOD13);
    ...
    auto multisolids_lod22 = extrude(
        arrangement, floor_elevation, attr_val3dity_lod22, attr_rmse_lod22,
        SegmentRasteriser.get(), PlaneDetector.get(), LOD22);
