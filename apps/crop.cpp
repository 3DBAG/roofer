...
void print_help(std::string program_name) {
  ...
  fmt::print(
      "   -r, --rasters                Output rasterised building "
      "pointclouds\n");
  fmt::print("   -m, --metadata               Output metadata.json file\n");
  fmt::print("   -i, --index                  Output index.gpkg file\n");
  fmt::print(
      "   -a, --all                    Output files for each candidate point "
      "cloud instead of only the optimal candidate\n");
}
...
int main(int argc, const char* argv[]) {
  ...
  for (auto& ipc : input_pointclouds) {
    ...
    for (unsigned i = 0; i < N_fp; ++i) {
      roofer::RasterisePointcloud(ipc.building_clouds[i], footprints[i],
                                  ipc.building_rasters[i], cellsize);
      ipc.nodata_fractions[i] =
          roofer::computeNoDataFraction(ipc.building_rasters[i]);
      ipc.pt_densities[i] =
          roofer::computePointDensity(ipc.building_rasters[i]);
      ...
      if (low_lod) {
        ...
        roofer::compute_nodata_circle(ipc.building_clouds[i], footprints[i],
  ...
  if (input_pointclouds.size() > 1) {
    ...
    for (unsigned i = 0; i < N_fp; ++i) {
      is_mutated[i] =
          roofer::isMutated(input_pointclouds[0].building_rasters[i],
  ...
  for (unsigned i = 0; i < N_fp; ++i) {
    ...
    roofer::PointCloudSelectResult sresult =
        roofer::selectPointCloud(candidates, select_pc_cfg);
    ...
    if (sresult.explanation ==
        roofer::PointCloudSelectExplanation::PREFERRED_AND_LATEST)
      pc_select.push_back("PREFERRED_AND_LATEST");
    else if (sresult.explanation ==
             roofer::PointCloudSelectExplanation::PREFERRED_NOT_LATEST)
      pc_select.push_back("PREFERRED_NOT_LATEST");
    else if (sresult.explanation ==
             roofer::PointCloudSelectExplanation::LATEST_WITH_MUTATION)
      pc_select.push_back("LATEST_WITH_MUTATION");
    else if (sresult.explanation == roofer::PointCloudSelectExplanation::
                                        _HIGHEST_YET_INSUFFICIENT_COVERAGE)
      pc_select.push_back("_HIGHEST_YET_INSUFFICIENT_COVERAGE");
    else if (sresult.explanation ==
             roofer::PointCloudSelectExplanation::_LATEST) {
    ...
    }
    {
      ...
      std::string fp_path =
          fmt::format(building_gpkg_file_spec, fmt::arg("bid", bid),
      ...
      for (auto& ipc : input_pointclouds) {
        ...
        std::string pc_path = fmt::format(
            building_las_file_spec, fmt::arg("bid", bid),
            fmt::arg("pc_name", ipc.name), fmt::arg("path", output_path));
        std::string raster_path = fmt::format(
            building_raster_file_spec, fmt::arg("bid", bid),
            fmt::arg("pc_name", ipc.name), fmt::arg("path", output_path));
        std::string jsonl_path = fmt::format(
            building_jsonl_file_spec, fmt::arg("bid", bid),
            fmt::arg("pc_name", ipc.name), fmt::arg("path", output_path));
        ...
        if (!only_write_selected) {
          ...
          std::string config_path = fmt::format(
              building_toml_file_spec, fmt::arg("bid", bid),
              fmt::arg("pc_name", ipc.name), fmt::arg("path", output_path));
  ...
  if (write_index) {
    std::string index_file =
        fmt::format(index_file_spec, fmt::arg("path", output_path));
    ...
    for (auto& [name, pathsvec] : jsonl_paths) {
      if (pathsvec.size() != 0) {
        std::string jsonl_list_file =
            fmt::format(jsonl_list_file_spec, fmt::arg("path", output_path),
  ...
  if (write_metadata) {
    ...
    std::string metadata_json_file =
        fmt::format(metadata_json_file_spec, fmt::arg("path", output_path));
